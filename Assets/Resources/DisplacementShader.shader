Shader "Unlit/DisplacementShader"
{
	Properties
	{
		...
		_BaseHeight ("BaseHeight", float) = 0.6
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				i.uv.x += dis * 0.006 * maskValue * _Turbulence;
				i.uv.y += dis * 0.006 * maskValue * _Turbulence;

				fixed4 col = tex2D(_MainTex, fixed2(i.uv.x, i.uv.y)); 
				fixed waterColBlendFac = maskValue * 0.5;				
				...
				fixed topColorBlendFac = isNearTop * maskValue;
